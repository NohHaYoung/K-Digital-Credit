...
def read_data():
    ...
    for i in range(1, len(tokens)):
        ...
        if token[2] is '0':
            training_sentences[0].append(token[1])
        else:
            training_sentences[1].append(token[1])
...
def calculate_doc_prob(training_sentence, testing_sentence, alpha):
    ...
    # 전체 토큰의 개수
    total_token = 0
    for key in training_model:
        total_token += training_model[key]
    print(total_token)
    ...
    for i in testing_model:
        try:
            if i in training_model:
                logprob += math.log(training_model[i]/total_token)
            else:
